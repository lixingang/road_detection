...
def preprocess(img, thresh):
    #print(sys._getframe().f_code.co_name, img)
...
def make_skeleton(datapath, debug, threshes, fix_borders,reverse):
    replicate = 5
    clip = 2
    rec = replicate + clip
...
def build_graph_single(datapath, debug=False, threshes=0.5, add_small=True, fix_borders=False, reverse=False,inputTiff=None):
    city = os.path.splitext(datapath)[0].split('/')[-1]
    img_copy, ske = make_skeleton(datapath, debug, threshes, fix_borders, reverse)
    if ske is None:
        print(datapath)
        print('[I] ske is None')
        return city, [linestring.format("EMPTY")]
    ...
    for w in node_lines:
        ...
        for s, e in pairwise(w):
            ...
            for ix, val in enumerate(vals):
                ...
                #print(ps)
                ps_[:, 1] = trans[0]*1.0 + ps[:, 1]*1.0 * trans[1] + ps[:, 0]*1.0 * trans[2]
                ps_[:, 0] = trans[3]*1.0 + ps[:, 1]*1.0 * trans[4] + ps[:, 0]*1.0 * trans[5]
                #print(ps_)
                line_strings = ["{1:.10f} {0:.10f}".format(*c.tolist()) for c in ps_]
                if ix == 0:
                    coord_list.extend(line_strings)
                else:
                    additional_paths.append(line_strings)
...
if __name__ == "__main__":
    ...
    datapath = '../data/testing_img1325.png'
    im = cv2.imread(datapath)
    print(im)
    data = preprocess(im,0.6)
    data = np.uint8(data)
    data = cv2.cvtColor(data,cv2.COLOR_BGR2GRAY)
    data = skeletonize(data)
    cv2.imwrite('result.png',np.uint8(data)*255)
