...
path_apls = '/home/lixg/script/apls/apls'
...
def clean_sub_graphs(G_, min_length=10, weight='length', verbose=False,
    ...
    for G_sub in sub_graphs:
        ...
        # get all lenghts
        lens = []
        for u,v in all_lengths.items():
            for uprime, vprime in v.items():
                lens.append(vprime)
                if super_verbose:
                    print( "u, v", u,v)
...
def create_gt_graph(geoJson, im_test_file, network_type='all_private',
                 ...
                 verbose=False):
    '''Ingest graph from truthGeoJson file and refine'''
...
def path_sim_metric(all_pairs_lengths_gt, all_pairs_lengths_prop, 
    ...
    #for start_node, paths in all_pairs_lengths.items():
...
def main():
    ...
    parser.add_argument('--test_method', default='test_geojson_multi', type=str,
    ...
    if args.test_method == 'test_geojson_multi':
        ...
        outputs_ims = '/home/lxg/data/road/spacenet/logs_Vegas/logs_ours/apls_outputs/outputs_ims/'
        truth_dir = '/home/lxg/data/road/spacenet/logs_Vegas/logs_ours/apls_outputs/labels_geojson/'
        prop_dir = '/home/lxg/data/road/spacenet/logs_Vegas/logs_ours/apls_outputs/prop_geojson/'
        gt_list, gt_raw_list, gp_list, root_list = [], [], [], []

        name_list = os.listdir(prop_dir)
        print('there are ',len(name_list),'files ')
        for f in name_list:
            # skip non-truthGeoJson files
            if not f.endswith('.geojson'):
                continue
            filename = f.split('.')[0]
            print( "\nfilename:", filename)
            prop_file= os.path.join(prop_dir, f)
            gt_file = os.path.join(truth_dir, filename + '.geojson')
            ...
            root_list.append(filename)
        
    ###################
    # ingest ground truth and propoal geojsons created in qgis
    if args.test_method == 'test_geojson':
        outroot = 'test0'
        gt_file = os.path.join(path_apls, 'sample_data/img1326_prop.geojson')
        prop_file = os.path.join(path_apls, 'sample_data/img1326_prop.geojson')
    ...
    for i,[outroot, G_gt_init, G_p_init] in enumerate(zip(root_list, gt_list, gp_list)):
        ...
        ##################
        # make dirs
        outdir_base = outputs_ims
        outdir = os.path.join(outdir_base, outroot)
        d_list = [outdir_base, outdir_base, outdir]
        for p in d_list:
    ...
    print(('total apls: ',apls_metric_sum*1.0 / number_of_apls))
